...
if (isset($_POST['login']) && isset($_POST['password']) && $_POST['login'] != '' && $_POST['password'] != '') {
    ...
    $prepare_statement = $connection->prepare('select idU, NomU, PrenomU, PasswordHashU from User where LoginU = ?');
    $prepare_statement->execute(array($_POST['login']));
    $prepare_statement_result = $prepare_statement->fetch();
    if(password_verify($_POST['password'], $prepare_statement_result[3])){// méthode de verification plus sure
